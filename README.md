...
[`PARSECS/`](PARSECS/) tree is the Low Level Substack (Layer 1, 2 and 3) ported
from `coretx-motherboard/rpi_motherboard/SPI` (master) and
`coretx_commboard/commboard_freertos_stm32/SPI` (slave). Layer 1 is the only
node-specific piece: it keeps the I2C DMA byte pump. Layer 2 frames
`SPI_BASE_FRAME` (SOF / LEN / SEQ / TYPE / DATA / CRC16) and Layer 3 handles
ACK/NACK. The demo queues `PING` (master) / `PONG` (slave) through
`PARSECS_TRANSMIT_APP` and logs `PARSECS_RECEIVE_APP` over USB CDC.
...
### PARSECS Low Level over I2C DMA (`main`)

The current branch. `PARSECS_LowLevelTask` runs L3 TX, L2 TX, L1, L2 RX, L3 RX
each tick. Layer 1 still moves **one byte each way per I2C round** with
`HAL_I2C_*_Seq_*_DMA` (DMA1 Stream0/Stream6). Layer 2 turns those bytes into
framed packets; Layer 3 ACKs a good DATA frame.

`DemoTask_Init` registers the I2C peer (`PARSECS_Add_Slave` no-op chip-select
on the master). `MyDemoTask` queues one payload and prints received frames.
...
job order, test payload, and error handling.

LED feedback (GPIOD, same mapping on both boards):

| LED | Color | Meaning |
|-----|-------|---------|
| LD4 | Green | Heartbeat — L1 round complete, or APP frame received |
...
```
PARSECS/              Shared Low Level Substack (L1 I2C DMA, L2, L3, LowLevelAPI)
...
PARSECS_RT_slave/     STM32CubeIDE project — I2C slave
  Application/
    demo_task.c/h     TRANSMIT_APP / RECEIVE_APP demo
