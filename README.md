...
Three STM32F407G-DISC1 boards communicating over I2C using FreeRTOS: one master and two slaves. The shared
...
`PARSECS_Protocol_USER_Receive`). The demo loops a GetRequest / GetResponse
to both slaves (TypeID `0x01`; comm answers `42`, mobility answers `43`), then
holds SCL and SDA low before the next round.
...
PB6/PB7 (SCL/SDA) tied together across the master and both slaves with a common GND, I2C1 at 400 kHz, 7-bit addressing. Comm slave address `0x08`, mobility slave address `0x09`. See [DIAGRAMS.md](DIAGRAMS.md) for the full pinout diagram.

---

## Demos

### PARSECS High Level over I2C DMA (`feature/dual-slave-get-loop`)

The current branch. `PARSECS_LowLevelTask` runs L3 TX, L2 TX, L1, L2 RX, L3 RX
each tick. `PARSECS_Protocol_Interface_Task` then runs L4/L6/L7 on both I2C
peers. Layer 1 still moves **one byte each way per I2C round** with
`HAL_I2C_*_Seq_*_DMA` (DMA1 Stream0/Stream6), using that slave's I2C address.

`PARSECS_Protocol_Interface_Task_Init` registers comm (`0x08`) and mobility
(`0x09`) with `PARSECS_Add_Slave` (no-op chip-select). `MyDemoTask` uses only
the High Level USER API — it does not call `PARSECS_TRANSMIT_APP` /
`PARSECS_RECEIVE_APP`. Each cycle is GetRequest/GetResponse for comm, then
mobility, a short settle so the last ACK can leave, then SCL and SDA held low
for `DEMO_BUS_HOLD_MS` (500 ms). A missing slave times out and the loop
continues.
...
PARSECS_RT_slave/     STM32CubeIDE project — I2C slave
  Application/
    demo_task.c/h     USER_Send / USER_Receive GetRequest loop
    slave_identity.h  Slave image select (comm 0x08 or mobility 0x09)
...
Open `PARSECS_RT_master/` and `PARSECS_RT_slave/` as separate projects in STM32CubeIDE and build each for the `Debug` configuration. Flash the master once. Flash the slave project twice: the default image is comm (`PARSECS_SLAVE_INDEX` 0, address `0x08`); rebuild with `-DPARSECS_SLAVE_INDEX=1` (or edit the default in `PARSECS_RT_slave/Application/slave_identity.h`) for mobility (`0x09`). The `.ioc` `OwnAddress1` stays `0x08`; the slave `i2c.c` user section applies the selected address after `MX_I2C1_Init`.
